...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
 const mxArray * prhs[] )
{
  extern unsigned int sf_BenchMark_process_check_sum_call( int nlhs, mxArray *
   plhs[], int nrhs, const mxArray * prhs[] );

  if(sf_BenchMark_process_check_sum_call(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
unsigned int sf_process_autoinheritence_call( int nlhs, mxArray * plhs[], int
 nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_BenchMark_autoinheritance_info( int nlhs, mxArray *
   plhs[], int nrhs, const mxArray * prhs[] );
  if(sf_BenchMark_autoinheritance_info(nlhs,plhs,nrhs,prhs)) return 1;
  return 0;
}
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray
 * plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_BenchMark_get_eml_resolved_functions_info( int nlhs,
  ...
  if(strcmp(machineName, "BenchMark") == 0) {
    const mxArray *newRhs[2] = { NULL, NULL };
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_BenchMark_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_BenchMark_method_dispatcher(SimStruct *S, unsigned int
...
{
  if(!strcmp(machineName,"BenchMark")) {
    return(sf_BenchMark_method_dispatcher(simstructPtr,chartFileNumber,method,data));
  }
  return 0;
}
extern void BenchMark_terminator(void);
void sf_machine_global_terminator(void)
{
  if(sfMachineGlobalTerminatorCallable) {
    ...
    BenchMark_terminator();
    sf_debug_terminate();
  }
  return;
}
extern void BenchMark_initializer(void);
extern void BenchMark_register_exported_symbols(SimStruct* S);
extern void BenchMark_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  BenchMark_register_exported_symbols(S);
}
