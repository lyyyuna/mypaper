  ...
  const mxArray * prhs[] )
{
  ...
  if (sf_BenchMark_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
  ...
  nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_BenchMark_autoinheritance_info( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_BenchMark_autoinheritance_info(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
  ...
  method, void *data)
{
  if (!strcmp(machineName,"BenchMark")) {
    return(sf_BenchMark_method_dispatcher(simstructPtr,chartFileNumber,
