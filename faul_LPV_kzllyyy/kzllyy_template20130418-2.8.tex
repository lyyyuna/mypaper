...
错和主动容错，其中被动容错控制\!\cite{4}是考虑正常工作和故障状态的参数值设计
...
计策略难以着手\cite{6,7}。无论是传统的鲁棒技术\cite{6}还是一般的增益调度
控制\cite{7}，对对象参数的变化和工作点的选取，要求都非常苛刻，不符合实
际情况。基于鲁棒控制的线性参数变化(Linear Parameter Varying, LPV)
增益调度技术对这种动态特性变化显著的风力系统会非常有效\cite{8,9}。对
风力系统来讲，鲁棒性和容错性都是必不可少的主要特性，而LPV控制
...
额定风速之上时，风机的恒功率控制\!\cite{11}。在实际运行和仿真中，虽然LPV的
...
设计\!\cite{14}，如图1所示，选取发电机转速及其积分值作为观测器
...
与LPV主动容错控制对比的是一个普通的PI调节器，其参数为$K_P=4.234,K_I=0.978$。
仿真时间$60s$，以下为仿真结果图。

\begin{center}
\includegraphics [scale=0.8,trim=0 0 0 0]{wind.eps}\\
\label{fig:wind}
{\fontsize{9.3pt}{11.6pt}\selectfont 图~2~~风速\\
Fig.~2~~Wind Velocity }
\end{center}
\begin{center}
\includegraphics [scale=0.8,trim=0 0 0 0]{omega_r_nofault.eps}\\
\label{fig:ono}
{\fontsize{9.3pt}{11.6pt}\selectfont 图~3~~容错控制下转子转速\\
Fig.~3~~Rotor Speed with Fault-Tolerant Control }
\end{center}
\begin{center}
\includegraphics [scale=0.8,trim=0 0 0 0]{omega_r_fault.eps}\\
\label{fig:ofault}
{\fontsize{9.3pt}{11.6pt}\selectfont 图~4~~无容错控制下转子转速\\
Fig.~4~~Rotor Speed without Fault-Tolerant Control }
\end{center}
\begin{center}
\includegraphics [scale=0.8,trim=0 0 0 0]{P_g_nofault.eps}\\
\label{fig:Pno}
{\fontsize{9.3pt}{11.6pt}\selectfont 图~5~~容错控制下电功率\\
Fig.~5~~Electrical Power with Fault-Tolerant Control }
\end{center}
\begin{center}
\includegraphics [scale=0.8,trim=0 0 0 0]{P_g_fault.eps}\\
\label{fig:Pfault}
{\fontsize{9.3pt}{11.6pt}\selectfont 图~6~~无容错控制下电功率\\
Fig.~6~~Electrical Power without Fault-Tolerant Control }
\end{center}

图2所示的是风速变化曲线，风速在额定风速以上，此时控制的目标是实现
恒功率控制。图3-图6均为有故障时不同控制器的效果图。从图3和图4看，虽然
低速侧转子转速对比效果不明显，但经过传动机构后，差别就被
放大，体现在发电机产生的功率上。从图5和图6看，在没有容错控制器的
情况下，产生的功率波动为$4.78\times10^6W\sim4.82\times10^6W$，而有
容错控制的情况下，功率波动在$4.79\times10^6W\sim4.81\times10^6W$。从而
减轻对电网的压力。
