...
\hspace*{1.1em}$^{\dag}$通信作者. E-mail: wh033098@163.com; Tel.: +86 .\\
...
针对风机具有非线性和参数的不确定性的特征，提出了基于LPV增益调度
的风机主动容错控制方法，解决故障对
机组带来的不稳定性问题。基于LPV凸分解方法，将风机的非线性模
...
{\CJKfamily{hei} 关键词:} 风机; 主动容错控制; LPV增益调度技术;
凸分解
...
fault tolerant control technology  was proposed to resolve the
 dynamic stability problems, which based on the
LPV gain scheduling technology. The linearized model around the
operating point of wind turbines and fault were converted into convex
...
命等等\!\cite{2}。由于大多数风机安装在远离人烟的地区，将容错控制技术引入风力发
...
点附近发生非线性变化，同时缺少风力机精确的数学模型，传统的控制器设
计策略难以胜任\cite{6,7}。无论是传统的鲁棒技术\cite{6}还是一般的增益调度
控制\cite{7}，对模型参数变化和工作点选取的要求，都非常苛刻，不符合实
际情况。基于鲁棒控制的线性参数变化(Linear Parameter Varying, LPV)
增益调度技术对这种动态特性变化显著的风力系统会非常有效\cite{8,9}。对
风力系统来讲，鲁棒性和容错性都是必不可少的特征，而LPV控制
...
额定风速以上时，风机恒功率控制的动静态特性\!\cite{11}。在实际运行和仿真中，虽然LPV的
综合过程中LMI迭代求解计算量大，但LPV容错控制器是离线计算，在线运行
时只需在每个采样周期，根据故障信息的变化，经过简单的算数运算，便可
得出随增益调度变量变化的控制器，涉及的运算量很小。另外，在
实际系统中，有限的采样周期内获得故障信息的精确估计是不太可能的，值
的庆幸的是Apkarian等人\!\cite{12}已经证实LPV对时变参数的估计误差并不敏感，不
...
\section{风机模型({\rm Wind Turbine Model})}
风机的非线性模型，即包括空气动力学子模型，传
...
\subsection{风机LPV模型 ({\rm LPV Model of Wind Turbine})}
...
输出$y=[\omega_g,y_\omega]^\mathrm{ T }$，控制器输出桨距参考值$\beta_{ref}$控制风机恒功率输出。本节设计
...
Fig.~1~~Structure of Fault-Tolerant Control }
\end{center}
...
恒功率控制。图3是发电机转速图，图3的第一张为正常情况的转速图，发电机转速维持
在$162rad/s$附近。图3的第二张和第三张分别为采用LPV容错控制器和PI控制器的转速图，
在第$30s$故障发生后，LPV容错控制器能有效抑制故障带来的影响。
图4是发电机功率，其第一张为正常情况功率图，第二第三张分别是LPV容错控制器和PI控制器
作用下功率图，且均在30s处发生故障。可以明显的看出，故障对LPV容错控制下的恒功率输出几乎
...
\end{center}
\begin{center}
\includegraphics [scale=0.8,trim=0 0 0 0]{omega_g_3.eps}\\
\label{fig:omegag}
{\fontsize{9.3pt}{11.6pt}\selectfont 图~3~~发电机转速\\
Fig.~3~~Generator Speed }
\end{center}
\begin{center}
\includegraphics [scale=0.8,trim=-10 0 0 11,clip=true]{P_g_3.eps}\\
\label{fig:Pg}
...
本文依据LPV系统的鲁棒$H\infty$控制原理，分别以输入风速、故障信息作为
变增益参数，建立
了风能转换系统故障模型的LPV控制方程，求解LMIs，得到系统
离线顶点控制器。最后在仿真模型中嵌入该LPV容错控制器，仿
真结果表明，所设计的控制器能有效地抑制故障带来的发电机
产生功率的波动，控制效果满意。
